...
pub fn main() !void {
    ...
    std.posix.unlink(socket_path) catch |unlink_error| {
        if (unlink_error != std.posix.UnlinkError.FileNotFound)
            return unlink_error;
    };
    ...
    const dev_uinput = try std.fs.openFileAbsolute("/dev/uinput", .{
        .mode = .write_only,
    });
    defer dev_uinput.close();
    ...
    if (std.posix.system.ioctl(
        ...
        return error.UI_SET_EVBIT_FAILED;
    ...
    inline for (.{
        ...
        uinput.KEY_RIGHT_UP,
        uinput.KEY_RIGHT_DOWN,
        uinput.KEY_LEFT_UP,
        uinput.KEY_LEFT_DOWN,
        uinput.KEY_ROOT_MENU,
        uinput.KEY_MEDIA_TOP_MENU,
        uinput.KEY_NUMERIC_11,
        uinput.KEY_NUMERIC_12,
        uinput.KEY_AUDIO_DESC,
        uinput.KEY_3D_MODE,
        uinput.KEY_NEXT_FAVORITE,
        uinput.KEY_STOP_RECORD,
        uinput.KEY_PAUSE_RECORD,
        uinput.KEY_VOD,
        uinput.KEY_UNMUTE,
        uinput.KEY_FASTREVERSE,
        uinput.KEY_SLOWREVERSE,
        uinput.KEY_DATA,
        ...
        if (std.posix.system.ioctl(
            ...
            return error.UI_SET_KEYBIT_FAILED;
    }
    ...
    inline for (.{
        ...
        if (std.posix.system.ioctl(
            ...
            return error.UI_SET_RELBIT_FAILED;
    }
    ...
    inline for (.{
        ...
        if (std.posix.system.ioctl(
            ...
            return error.UI_SET_ABSBIT_FAILED;
    }

    var device_name: [80]u8 = undefined;
    std.mem.copyForwards(u8, &device_name, "sparrow virtual device");
    if (std.posix.system.ioctl(
        dev_uinput.handle,
        uinput.UI_DEV_SETUP,
        &uinput.uinput_setup{
        ...
        return error.UI_DEV_SETUP_FAILED;

    if (std.posix.system.ioctl(
        dev_uinput.handle,
        uinput.UI_DEV_CREATE,
    ) != 0)
        return error.UI_DEV_CREATE_FAILED;

    var event = uinput.input_event{};
    const input_event_size = @sizeOf(uinput.input_event);
    while (true) {
        if (std.posix.system.recv(
            socket_fd,
            @ptrCast(&event),
            input_event_size,
            0,
        ) == @sizeOf(uinput.input_event))
            std.debug.print("Received type: {d}, code: {d}, value: {d}\n", .{
                event.type,
                event.code,
                event.value,
            });
